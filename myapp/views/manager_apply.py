...
def qingjia(request):
    ...
    manager_id = var['id']
    apply_qingjia=Absence.objects.filter(man_staffno=manager_id,abstatu="未审核")
...
def jiaban(request):
    ...
    var = request.session.get('info')
    manager_id = var['id']
    apply_jiaban=Overtime.objects.filter(man_staffno=manager_id,overstatu="未审核")
...
def baoxiao(request):
    ...
    var = request.session.get('info')
    manager_id = var['id']
    apply_baoxiao=Reimbursement.objects.filter(man_staffno=manager_id,reimstatu="未审核")
...
def chuchai(request):
    ...
    var = request.session.get('info')
    manager_id = var['id']
    apply_chuchai=Businesstrip.objects.filter(man_staffno=manager_id,btstatu="未审核")
...
def cizhi(request):
    ...
    var = request.session.get('info')
    manager_id = var['id']
    apply_cizhi=Leave.objects.filter(man_staffno=manager_id,leavestatu="未审核")
